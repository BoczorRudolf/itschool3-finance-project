...
class AssetInfoBase(OrmModel):
    ...
    name: str = Field(description="Name of a company based on the ticker given")
    country: str = Field(
...
class AssetInfoUser(AssetInfoBase):
    units: float = Field(description="Number of stocks or security a user has")
...
class AssetInfoPrice(AssetInfoBase):
    ...
    avg_today_price: float = Field(default=None, description="Average today price")
